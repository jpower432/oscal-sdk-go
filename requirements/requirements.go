...
func Components(ctx context.Context, componentId string, store rules.Store, requirements RequirementMap) ([]extensions.RuleSet, error) {
	allMappedRules := NewSet[string]()
	allParameters := make(map[string]string)
	for _, requirement := range requirements {
		for _, mappedRule := range requirement.MappedRules {
			allMappedRules.Add(mappedRule)
		}
	...
	for _, ruleSet := range componentRuleSets {
		if !allMappedRules.Has(ruleSet.Rule.ID) {
			continue
		}
		ruleSet = applyParameterSettings(allParameters, ruleSet)
...
func applyParameterSettings(allParameterOverrides map[string]string, ruleSet extensions.RuleSet) extensions.RuleSet {
	if len(allParameterOverrides) > 0 && ruleSet.Rule.Parameter != nil {
		selectedValue, ok := allParameterOverrides[ruleSet.Rule.Parameter.ID]
		if ok {
